...
async function createCart(addrecipes, success, fail) {
  await api.post(`/api/cart`, addrecipes).then(success).catch(fail)
}

// 장바구니 레시피 삭제
async function deleteCart(id, success, fail) {
  await api
    .delete(`/api/cart`, { data: { recipeId: id } })
    .then(success)
    .catch(fail)
}
...
async function getCartIngredient(success, fail) {
  await api.get(`/api/cart-ingredient`).then(success).catch(fail)
}

// 장바구니 추가재료 추가
async function createCartIngredient(createObject, success, fail) {
  await api.post(`/api/cart-ingredient`, createObject).then(success).catch(fail)
}
...
async function startCart(success, fail) {
  await api.post(`/api/cart-history`).then(success).catch(fail)
}
